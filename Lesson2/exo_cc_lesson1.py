...
def pig_latin(text):
    string_out = ' '.join([word[1]+ word[2:] + word[:1].lower() + "ay" for word in text.split(' ')])
    return string_out.capitalize()
...
def extract_data(data):
    list_of_dict = []
    for cityname, city_weather_data in data.items():
        country = city_weather_data['metadata']['country']
        date = city_weather_data['weather_list'][0]['dt_txt']
        temp = city_weather_data['weather_list'][0]['main']['temp'] - 273.15
        temp_dict = {"name":cityname, "country":country, "date":date, "temp":temp}
