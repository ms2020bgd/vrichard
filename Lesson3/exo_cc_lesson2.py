...
def get_prices_from_url(url):
    ...
    types_prices = soup.find("div", class_="pricing-tables").find_all(class_="pure-u-1")
    for type_price in types_prices:
        dict_type_price = {}
        name = type_price.find("h2").text
        price = type_price.find("span", class_="pricing-table-price").text.split()[0]
        storage = type_price.find("li", string=re.compile('GB|TB')).text.split()[0]
        databases = int(type_price.find("li", string=re.compile('database')).text.split()[0])
        dict_type_price['price'] = price
        dict_type_price['storage'] = storage
        dict_type_price['databases'] = databases
        dict_general[name] = dict_type_price
    return dict_general
...
def extract_beer_list_infos(url):
    ...
    for beer in beers["items"]:
        beer_list_infos.append(extract_beer_infos("https://www.beerwulf.com"+beer["contentReference"]))
    return beer_list_infos
